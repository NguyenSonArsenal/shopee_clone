...
export default function ApiDocCreatePage() {
  const {showToast} = useToast()
  const router = useRouter()
  const queryClient = useQueryClient()

  const {control, handleSubmit, setError, formState: {errors}} = useForm<ApiDocFormValues>({
  ...
  const {mutate, isPending} = useMutation({
    mutationFn: (formData: ApiDocFormValues) => apiDocApi.store(formData),
    onSuccess: async (data) => {
      await queryClient.invalidateQueries({queryKey: ["api_doc_list"], refetchType: 'all'})
      router.push(`${ROUTES.SWAGGER}/${data.id}`)
      showToast("success", transMessage('store_success', {label: url}))
    },
    onError: (err: any) => {
  ...
  return (
    <AdminLayout breadcrumb={apiDoc.create} hideSidebar>
      <div className="card">
            ...
            <div className="frow c2">
              <div className="field">
                ...
                <Controller
                  ...
                  control={control}
                  render={({field}) => (
                    <input type="text" placeholder="Nhập tên module" {...field} value={field.value ?? ""} maxLength={LENGTH.api_doc.module}/>
                  )}
                />
            ...
            <div className="frow c1">
              <div className="field">
                ...
                <Controller
                  ...
                  control={control}
                  render={({field}) => (
                    <input type="text" placeholder="Nhập url, ví dụ: login" {...field} value={field.value ?? ""} maxLength={LENGTH.api_doc.url}/>
                  )}
                />
            ...
            <div className="frow c1">
              <div className="field">
                ...
                <Controller
                  ...
                  control={control}
                  render={({field}) => (
                    <input type="text" placeholder="Nhập mô tả" {...field} value={field.value ?? ""} maxLength={LENGTH.api_doc.description}/>
                  )}
                />
            ...
            <div className="frow c1">
              <div className="field">
                ...
                <Controller
                  ...
                  control={control}
                  render={({field}) => (
                    <textarea placeholder="Nhập tham số" rows={4} className={styles.codeTextarea} {...field} value={field.value ?? ""}/>
                  )}
                />
            ...
            <div className="frow c1">
              <div className="field">
                ...
                <Controller
                  ...
                  control={control}
                  render={({field}) => (
                    <textarea placeholder="Nhập curl mẫu" rows={4} className={styles.codeTextarea} {...field} value={field.value ?? ""}/>
                  )}
                />
            ...
            <div className="frow c1">
              <div className="field">
                ...
                <Controller
                  ...
                  control={control}
                  render={({field}) => (
                    <textarea placeholder="Nhập response mẫu" rows={6} className={styles.codeTextarea} {...field} value={field.value ?? ""}/>
                  )}
                />
