...
<title>Mã xác thực {{ $siteName }}</title>
...
<style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .intro { font-size:14px; color:#555; margin-bottom:26px; }

    /* Khối mã kích hoạt */
    .code-box { background:#f5f5f7; border-radius:10px; padding:26px 20px; text-align:center; margin-bottom:22px; }
    .code { font-size:40px; font-weight:700; letter-spacing:10px; color:#b20707; font-family:'Roboto',-apple-system,'Segoe UI',Arial,sans-serif; }
    .expire { font-size:14px; color:#555; margin-bottom:26px; line-height:1.7; }
    .expire strong { color:#e74c3c; }

    /* Nút hành động (tùy chọn) */
    .action-wrap { text-align:center; margin-bottom:28px; }
    .btn-action { display:inline-block; background:#e74c3c; color:#fff !important; text-decoration:none; font-size:15px; font-weight:700; padding:13px 34px; border-radius:8px; }

    .divider { height:1px; background:#eee; margin:8px 0 22px; }
    .note { font-size:14px; color:#555; line-height:1.7; margin-bottom:24px; }
    .sign { font-size:14px; color:#555; margin-bottom:28px; }
    .sign strong { color:#333; }
    ...
    @media (max-width:600px) {
        ...
        .code { font-size:30px; letter-spacing:7px; }
    }
</style>
</head>
<body>
<div class="wrap">
    ...
    {{-- ── Thân: khối OTP ── --}}
    <div class="body">
        <p class="greeting">Xin chào <b>{{ $name ?? 'bạn' }}</b>,</p>
        <p class="intro">
            Bạn đang thực hiện xác thực tài khoản trên <b>{{ $siteName }}</b>.
            Vui lòng sử dụng mã kích hoạt bên dưới để hoàn tất quá trình:
        </p>
        ...
        <p class="expire">
            ...
            Vui lòng không chia sẻ mã này cho bất kỳ ai, kể cả nhân viên của <b>{{ $siteName }}</b>.
        </p>

        {{-- Nút "Xác nhận ngay" chỉ hiện khi có URL hành động (tùy chọn). --}}
        @if(!empty($actionUrl))
        <div class="action-wrap">
            <a href="{{ $actionUrl }}" class="btn-action">Xác nhận ngay</a>
        </div>
        @endif
    ...
    <div style="display:none;max-height:0;max-width:0;overflow:hidden;opacity:0;color:transparent;font-size:1px;line-height:1px;mso-hide:all;">
        {{ $otp }}-{{ now()->format('YmdHisu') }}-{{ \Illuminate\Support\Str::random(12) }}
    </div>
</div>
</body>
</html>
